...
final readonly class ProjectArchivedOutboxMessageHandler
{
    ...
    public function __invoke(ProjectArchivedMessage $message): void
    {
        $this->transactional->transactional(function () use ($message): void {
            ...
            $this->logger->info('ProjectArchived outbox message consumed.', [
                'outbox_id' => $message->outboxId,
                'event_id' => $message->eventId,
                'project_id' => $message->projectId,
                'archived_at' => $message->archivedAt->format(DATE_ATOM),
            ]);
