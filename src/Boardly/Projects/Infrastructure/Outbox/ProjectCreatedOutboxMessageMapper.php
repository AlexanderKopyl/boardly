...
final readonly class ProjectCreatedOutboxMessageMapper implements OutboxMessageMapperInterface
{
    private const EVENT_TYPE = 'projects.project_created';
    ...
    public function map(OutboxRecord $record): object
    {
        ...
        if (null !== $record->aggregateId && $record->aggregateId !== $projectId) {
            throw new \InvalidArgumentException(sprintf(
                'Project created outbox record aggregateId "%s" must match payload project_id "%s".',
                $record->aggregateId,
                $projectId,
            ));
        }
        ...
        return new ProjectCreatedMessage(
            ...
            projectId: $projectId,
            ownerAccountId: $this->payloadNonEmptyString($record, 'owner_account_id'),
            name: $this->payloadNonEmptyString($record, 'name'),
            iconKey: $this->payloadNonEmptyString($record, 'icon_key'),
            createdAt: $this->payloadDateTime($record, 'created_at'),
        );
    }
    ...
    private function payloadNonEmptyString(OutboxRecord $record, string $key): string
    {
        ...
        if (!is_string($value) || '' === trim($value)) {
            throw new \InvalidArgumentException(sprintf(
                'Project created outbox record payload field "%s" must be a non-empty string.',
                $key,
            ));
        }
    ...
    private function payloadDateTime(OutboxRecord $record, string $key): \DateTimeImmutable
    {
        ...
        if (!is_string($value) || '' === trim($value)) {
            throw new \InvalidArgumentException(sprintf(
                'Project created outbox record payload field "%s" must be a valid date/time string.',
                $key,
            ));
        }
