...
- [x] Wire Tailwind CSS into the `frontend/` workspace and confirm the app can compile utility classes.
- [x] Put the semantic CSS token baseline on a clean foundation in `frontend/src/app/globals.css` while keeping only true global/base rules there.
- [x] Replace `frontend/src/shared/lib/cn.ts` with a class-composition helper suitable for shared primitives.
