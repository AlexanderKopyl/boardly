...
use App\Boardly\Projects\Infrastructure\Outbox\ProjectArchivedOutboxMessageHandler;
...
final class ProjectArchivedOutboxMessageHandlerTest extends TestCase
{
    ...
    public function testHandlerAcceptsProjectArchivedMessage(): void
    {
        $method = new ReflectionMethod(ProjectArchivedOutboxMessageHandler::class, '__invoke');
        $parameters = $method->getParameters();

        self::assertCount(1, $parameters);
        self::assertSame(ProjectArchivedMessage::class, (string) $parameters[0]->getType());
    }

    public function testHandlerDoesNotDependOnGenericOutboxMessageOrEventTypeFiltering(): void
    {
        $source = file_get_contents(dirname(__DIR__, 5).'/src/Boardly/Projects/Infrastructure/Outbox/ProjectArchivedOutboxMessageHandler.php');
        ...
        self::assertStringNotContainsString('projects.project_archived', $source);
    }

    public function testProcessesConcreteMessageInsideTransaction(): void
    {
        $operationLog = [];
        $transactional = new ProjectArchivedRecordingTransactional($operationLog);
        $processedMessages = new ProjectArchivedFakeProcessedMessageStore($transactional, $operationLog);
        $logger = new ProjectArchivedRecordingLogger($transactional, $operationLog);
        ...
        self::assertSame([[self::EVENT_ID, ProjectArchivedOutboxMessageHandler::class]], $processedMessages->tryStartCalls);
        self::assertSame([[self::EVENT_ID, ProjectArchivedOutboxMessageHandler::class]], $processedMessages->markProcessedCalls);
        self::assertCount(1, $logger->records);
        self::assertSame('info', $logger->records[0]['level']);
        self::assertSame('ProjectArchived outbox message consumed.', $logger->records[0]['message']);
        self::assertSame([
    ...
    public function testDuplicateDeliveryIsSkippedWhenTryStartReturnsFalse(): void
    {
        $operationLog = [];
        $transactional = new ProjectArchivedRecordingTransactional($operationLog);
        $processedMessages = new ProjectArchivedFakeProcessedMessageStore($transactional, $operationLog, tryStartResult: false);
        $logger = new ProjectArchivedRecordingLogger($transactional, $operationLog);
    ...
    public function testMarkProcessedIsNotCalledWhenSideEffectThrows(): void
    {
        ...
        $transactional = new ProjectArchivedRecordingTransactional($operationLog);
        $processedMessages = new ProjectArchivedFakeProcessedMessageStore($transactional, $operationLog);
        $logger = new ProjectArchivedRecordingLogger($transactional, $operationLog, throwOnInfo: true);
    ...
    public function testLogContextContainsOnlySafeFields(): void
    {
        ...
        self::assertSame(['outbox_id', 'event_id', 'project_id', 'archived_at'], array_keys($logger->records[0]['context']));
    }

    private function handler(
        ProjectArchivedRecordingLogger $logger,
        ProjectArchivedRecordingTransactional $transactional,
        ProjectArchivedFakeProcessedMessageStore $processedMessages,
    ): ProjectArchivedOutboxMessageHandler {
        return new ProjectArchivedOutboxMessageHandler($logger, $transactional, $processedMessages);
    }

    private function message(): ProjectArchivedMessage
    {
        return new ProjectArchivedMessage(
            outboxId: 'outbox-record-id',
            eventId: self::EVENT_ID,
            projectId: self::PROJECT_ID,
            archivedAt: new \DateTimeImmutable('2026-05-07T09:10:00+00:00'),
        );
    }
}

final class ProjectArchivedRecordingTransactional implements TransactionalInterface
{
...
final class ProjectArchivedFakeProcessedMessageStore implements ProcessedMessageStoreInterface
{
    /**
    ...
    public function __construct(
        private readonly ProjectArchivedRecordingTransactional $transactional,
        array &$operationLog,
...
final class ProjectArchivedRecordingLogger extends AbstractLogger
{
    /**
