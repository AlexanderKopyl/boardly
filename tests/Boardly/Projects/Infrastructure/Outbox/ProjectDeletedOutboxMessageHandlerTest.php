...
use App\Boardly\Projects\Infrastructure\Outbox\ProjectDeletedOutboxMessageHandler;
...
final class ProjectDeletedOutboxMessageHandlerTest extends TestCase
{
    ...
    public function testHandlerAcceptsProjectDeletedMessage(): void
    {
        $method = new ReflectionMethod(ProjectDeletedOutboxMessageHandler::class, '__invoke');
        $parameters = $method->getParameters();

        self::assertCount(1, $parameters);
        self::assertSame(ProjectDeletedMessage::class, (string) $parameters[0]->getType());
    }

    public function testHandlerDoesNotDependOnGenericOutboxMessageOrEventTypeFiltering(): void
    {
        $source = file_get_contents(dirname(__DIR__, 5).'/src/Boardly/Projects/Infrastructure/Outbox/ProjectDeletedOutboxMessageHandler.php');
        ...
        self::assertStringNotContainsString('projects.project_deleted', $source);
    }

    public function testProcessesConcreteMessageInsideTransaction(): void
    {
        $operationLog = [];
        $transactional = new ProjectDeletedRecordingTransactional($operationLog);
        $processedMessages = new ProjectDeletedFakeProcessedMessageStore($transactional, $operationLog);
        $logger = new ProjectDeletedRecordingLogger($transactional, $operationLog);
        ...
        self::assertSame([[self::EVENT_ID, ProjectDeletedOutboxMessageHandler::class]], $processedMessages->tryStartCalls);
        self::assertSame([[self::EVENT_ID, ProjectDeletedOutboxMessageHandler::class]], $processedMessages->markProcessedCalls);
        self::assertCount(1, $logger->records);
        self::assertSame('info', $logger->records[0]['level']);
        self::assertSame('ProjectDeleted outbox message consumed.', $logger->records[0]['message']);
        self::assertSame([
    ...
    public function testDuplicateDeliveryIsSkippedWhenTryStartReturnsFalse(): void
    {
        $operationLog = [];
        $transactional = new ProjectDeletedRecordingTransactional($operationLog);
        $processedMessages = new ProjectDeletedFakeProcessedMessageStore($transactional, $operationLog, tryStartResult: false);
        $logger = new ProjectDeletedRecordingLogger($transactional, $operationLog);
    ...
    public function testMarkProcessedIsNotCalledWhenSideEffectThrows(): void
    {
        ...
        $transactional = new ProjectDeletedRecordingTransactional($operationLog);
        $processedMessages = new ProjectDeletedFakeProcessedMessageStore($transactional, $operationLog);
        $logger = new ProjectDeletedRecordingLogger($transactional, $operationLog, throwOnInfo: true);
    ...
    public function testLogContextContainsOnlySafeFields(): void
    {
        ...
        self::assertSame(['outbox_id', 'event_id', 'project_id', 'deleted_at'], array_keys($logger->records[0]['context']));
    }

    private function handler(
        ProjectDeletedRecordingLogger $logger,
        ProjectDeletedRecordingTransactional $transactional,
        ProjectDeletedFakeProcessedMessageStore $processedMessages,
    ): ProjectDeletedOutboxMessageHandler {
        return new ProjectDeletedOutboxMessageHandler($logger, $transactional, $processedMessages);
    }

    private function message(): ProjectDeletedMessage
    {
        return new ProjectDeletedMessage(
            outboxId: 'outbox-record-id',
            eventId: self::EVENT_ID,
            projectId: self::PROJECT_ID,
            deletedAt: new \DateTimeImmutable('2026-05-16T14:00:00+00:00'),
        );
    }
}

final class ProjectDeletedRecordingTransactional implements TransactionalInterface
{
...
final class ProjectDeletedFakeProcessedMessageStore implements ProcessedMessageStoreInterface
{
    /**
    ...
    public function __construct(
        private readonly ProjectDeletedRecordingTransactional $transactional,
        array &$operationLog,
...
final class ProjectDeletedRecordingLogger extends AbstractLogger
{
    /**
